...
function Skills() {
  ...
  useGSAP(() => {
    const skillTextSplit = new SplitText('.skill_head_text', {
      type: 'chars, words',
    });
          ...
          return (
            <li
              ...
              className={`skill_item ${
                stacks.includes(item.iconName) ? 'active' : ''
              }`}
              onClick={() => handleStacks(item.iconName)}
            >
              {item.icon} <span>{item.iconName}</span>
            </li>
          );
        })}
      </ul>
